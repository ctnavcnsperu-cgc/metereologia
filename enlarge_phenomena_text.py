...
# Pattern for the structure in index.html
pattern = r"""<div
...
\s+<span class="text-xs text-slate-900 dark:text-slate-100 font-bold truncate">(.*?)</span>
\s+</div>
...
def replacement(match):
    ...
    return f"""<div
                            ...
                            <div class="flex items-center gap-2 min-w-0 flex-grow">
                                <span class="font-bold text-red-700 dark:text-red-400 uppercase text-sm leading-none shrink-0 font-technical">{city}</span>
                                <span class="text-sm text-slate-900 dark:text-slate-100 font-bold truncate">{phenomenon}</span>
                            </div>
...
print("Updated index.html: Increased red and black text to text-sm.")
